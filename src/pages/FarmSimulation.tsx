...
const FarmSimulation = () => {
  ...
  const simZones = zones.slice(0, 2); // Only A and B for simulation
  ...
  const zone = simZones.find((z) => z.id === selectedZone)!;
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
            ...
            return (
              <div key={z.id} onClick={() => setSelectedZone(z.id)}
                ...
                <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-semibold ${isIrrigating ? "bg-farm-blue text-primary-foreground" : "bg-card/80 text-foreground"}`}>
                  {isIrrigating ? `💧 ${t("irrigating")}` : t("idle")}
                </div>
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card rounded-xl p-5">
          <h3 className="font-semibold text-sm mb-4">{t("irrigationControls")}</h3>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="bg-secondary/50 rounded-lg p-3">
              ...
              <span className={`ml-2 font-semibold ${irrigating[selectedZone] ? "text-farm-blue" : "text-muted-foreground"}`}>{irrigating[selectedZone] ? t("active") : t("idle")}</span>
            </div>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card rounded-xl p-5">
          <h3 className="font-semibold text-sm mb-4">Zone {selectedZone} {t("zoneDetails")}</h3>
          <div className="space-y-3">
            {[
              ...
              { label: t("irrigationStatus"), value: irrigating[selectedZone] ? `${t("active")} 💧` : t("idle") },
            ].map((item) => (
              <div key={item.label}>
